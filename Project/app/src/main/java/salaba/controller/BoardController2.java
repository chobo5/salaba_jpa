...
//      case 2: // 작성자만
//        if (loginUser == null || board.getWriter().getMemberNo() != loginUser.getMemberNo()) {
...
//          if (comment.getState() == '1' && replyList.isEmpty()) {
//            iterator.remove();
//            continue;
//          }
...
//      isLiked = boardService.isLiked(loginUser.getMemberNo(), boardNo);
//    }
//
...
//    if (old == null) {
//      throw new Exception("번호가 유효하지 않습니다.");
//    } else if (old.getWriter().getMemberNo() != loginUser.getMemberNo()) {
...
//    } else if (board.getWriter().getMemberNo() != loginUser.getMemberNo()) {
//      throw new Exception("권한이 없습니다.");
//    }
//
...
//    if( memberNoForAlarm != loginUser.getMemberNo() ){
...
//
//    if (oldComment.getWriter().getMemberNo() != loginUser.getMemberNo()) {
//      return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Access Denied");
//    }
//
//    comment.setWriter(loginUser);
...
//
//    if (loginUser.getMemberNo() != oldComment.getWriter().getMemberNo()) {
//      return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Access Denied");
//    }
//
...
//      // 게시글 작성자와 댓글 작성자가 다를 때만 알람 추가
//      Alarm alarm = new Alarm();
//      if( commentWriterNo != loginUser.getMemberNo() ){
...
//
//    if (oldReply.getWriter().getMemberNo() == loginUser.getMemberNo()) {
//      reply.setWriter(loginUser);
...
//    if (reply.getWriter().getMemberNo() == loginUser.getMemberNo()) {
...
//      int result = boardService.increaseLikeCount(boardNo, loginUser.getMemberNo()); // 추천수 증가: board_like 테이블에 추가
//      return ResponseEntity.ok(result);
//
//    } catch (Exception e) {
...
//    Member loginUser = (Member) session.getAttribute("loginUser");
//    try {
//      int result = boardService.decreaseLikeCount(boardNo, loginUser.getMemberNo()); // 추천수 감소: board_like 테이블에서 삭제
//
//      return ResponseEntity.ok(result);
//    } catch (Exception e) {
