...
import salaba.domain.auth.controller.AuthController;
import salaba.domain.auth.service.AuthService;
import salaba.domain.member.exception.AlreadyExistsException;
import salaba.domain.auth.security.jwt.util.JwtTokenizer;
...
class AuthControllerTest {
    ...
    public void 닉네임사용가능여부조회_실패() throws Exception {
        ...
        doThrow(AlreadyExistsException.class).when(authService).isExistingNickname(nickname);
    ...
    @Test
    @WithMockUser
    public void 이메일사용가능여부조회_실패() throws Exception {
        // given
        String email = "test@test.com";

        // when - 서비스의 동작을 목처리
        doThrow(AlreadyExistsException.class).when(authService).isExistingEmail(email);
        ...
        // 서비스 메서드가 호출되었는지 검증
        verify(authService, times(1)).isExistingEmail(email);
